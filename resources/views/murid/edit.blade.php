                ...
                <div class="card-body">
                    <form action="{{ route('murid.update',$datamurid->id)}}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control" name="tanggal_lahir" value="{{$datamurid->tanggal_lahir}}">
                            @error('tanggal_lahir')
                                <small style="color: red;">{{$message}}</small>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            <label>Alamat</label>
                              <textarea class="form-control" name="alamat" value="{{$datamurid->alamat}}"></textarea>
                             @error('alamat')
                                <small style="color: red;">{{$message}}</small>
                            @enderror
                        </div>
                        <div class="mb-3">
                            <label>Email</label>
                            <input type="email" class="form-control" name="email" value="{{$datamurid->email}}">
                             @error('email')
                                <small style="color: red;">{{$message}}</small>
                            @enderror
                        </div>
